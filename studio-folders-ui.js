...
function referenceCatalog(shared=globalAssets()){const local=db.assets.filter(a=>['scene','wardrobe'].includes(a.kind)).map(a=>({...a,id:'asset:'+a.id,tags:A.tags(a.category),category:a.kind==='scene'?'场景':a.wardrobeType==='inspiration'?'服装创意':'现有衣橱',notes:a.requirements||a.analysis||a.dimensions||a.inventory||'',link:'',files:[...(a.files||[]),...usableWardrobeOutfits(a).filter(o=>o.selected).map(o=>a.frames?.[o.frameIndex]).filter(Boolean),...[C.selected(a.proposal)].filter(Boolean)],usedIn:[],isProjectAsset:true}));return [...shared,...local];}
