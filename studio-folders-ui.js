...
function isCoverReferenceFolder(folder){return folder?.group==='cover'||/^(?:初瑞雪过往封面全集|历史参考｜(?:人物照片封面|纯字体模板)|(?:增量优化|审美升级)｜(?:新)?封面(?:建议|参考))$/.test(folder?.name||'');}
function coverReferenceFolders(){return referenceFolders().filter(isCoverReferenceFolder);}
function projectReferenceFolders(){return referenceFolders().filter(folder=>!isCoverReferenceFolder(folder));}
function saveReferenceFolders(items,space=scope){ensureScopedLibraries();const store=folderStoreKey(space),old=localStorage.getItem(store);if(old)localStorage.setItem(store+'_previous',old);localStorage.setItem(store,JSON.stringify(items));globalThis.markWorkspaceDirty?.(space);}
...
function usableWardrobeOutfits(a){return (a?.outfits||[]).filter(o=>{const text=[o.name,o.description,o.fullBodyReason].filter(Boolean).join(' ');return Number.isInteger(o.frameIndex)&&a.frames?.[o.frameIndex]&&!/(无法|不可)识别|看不清|仅剪影|大面积遮挡|动态模糊/.test(text);}).slice(0,3);}
...
function folderToolbar(){const folders=projectReferenceFolders();if(referenceFolder!=='all'&&referenceFolder!=='unfiled'&&!folders.some(f=>f.id===referenceFolder))referenceFolder='all';return `<div class="panel"><div class="section-head"><h2>项目参考文件夹</h2>${btn('新建项目文件夹','folder-new','',true)}</div><div class="actions">${btn('总审美库 · 全部资料','folder-open','data-id="all"')}${btn('尚未选入项目','folder-open','data-id="unfiled"')}${folders.map(f=>btn(esc(f.name),'folder-open',`data-id="${esc(f.id)}" class="${referenceFolder===f.id?'primary':''}"`)).join('')}</div><p class="muted">这里只放具体项目需要组合使用的参考。封面资料统一在下方“封面参考”分类及其子级中整理，不再占用项目文件夹入口。</p>${folders.filter(f=>f.id===referenceFolder).map(f=>`<h3>${esc(f.name)}</h3><p>${esc(f.description||'例如：双十一我冠是军。写下本项目要借鉴什么，以及哪些不要照搬。')}</p>${btn('从总审美库挑选／整理','folder-manage',`data-id="${f.id}"`)}`).join('')}<div class="actions" style="margin-top:14px">${btn('新建场景资料','new-asset','data-kind="scene"')}${btn('录入现有服装','new-asset','data-kind="wardrobe" data-wardrobe-type="inventory"')}${btn('新建AI服装创意','new-asset','data-kind="wardrobe" data-wardrobe-type="inspiration"')}</div></div>`;}
function referenceCatalog(shared=globalAssets()){const local=db.assets.filter(a=>['scene','wardrobe'].includes(a.kind)).map(a=>{const wardrobeImages=a.kind==='wardrobe'?usableWardrobeOutfits(a).map(o=>a.frames?.[o.frameIndex]).filter(Boolean):[];return {...a,id:'asset:'+a.id,tags:A.tags(a.category),category:a.kind==='scene'?'场景':a.wardrobeType==='inspiration'?'服装创意':'现有衣橱',notes:a.kind==='wardrobe'?(usableWardrobeOutfits(a)[0]?.recommendation||usableWardrobeOutfits(a)[0]?.description||a.requirements||''):a.requirements||a.analysis||a.dimensions||'',link:'',files:[...wardrobeImages,...(a.files||[]),...[C.selected(a.proposal)].filter(Boolean)],usedIn:[],isProjectAsset:true};});return [...shared,...local];}
...
function referenceContext(entity,kind='topic'){const r=resolvedReferences(entity,kind);return {folders:referenceFolders().filter(f=>r.folderIds.includes(f.id)).map(f=>({name:f.name,description:f.description})),aesthetic:r.shared.map(a=>({name:a.name,category:a.category,tags:a.tags,notes:a.notes,sourceLink:a.link,linkNotRead:!!a.link})),assets:r.assets.map(a=>({name:a.name,kind:a.kind,wardrobeType:a.wardrobeType,availability:a.kind==='wardrobe'?(a.wardrobeType==='inspiration'?'创意参考，尚未拥有':'现有衣橱，可直接取用'):undefined,dimensions:a.kind==='wardrobe'?undefined:a.dimensions,dimensionsConfirmed:a.kind==='wardrobe'?undefined:a.dimensionsConfirmed,requirements:a.requirements,inventory:a.kind==='wardrobe'?undefined:a.inventory,analysis:a.analysis,selectedOutfits:(a.kind==='wardrobe'?usableWardrobeOutfits(a):a.outfits||[]).filter(o=>o.selected)}))};}
function folderImageIDs(entity,kind='topic'){const r=resolvedReferences(entity,kind);return [...new Set([...r.assets.flatMap(a=>[C.selected(a.proposal),...(a.kind==='wardrobe'?usableWardrobeOutfits(a):a.outfits||[]).filter(o=>o.selected).map(o=>a.frames?.[o.frameIndex]),...(a.files||[])]),...r.shared.flatMap(a=>a.files||[])].filter(a=>C.assetOK(a,'image')).map(a=>a.localId))].slice(0,4);}
