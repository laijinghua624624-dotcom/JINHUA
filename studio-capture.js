...
(function(root){
  ...
  function quickDialog(prefill={}){
    const url=prefill.url||'',note=[prefill.note,prefill.title].filter(Boolean).join(noteJoin(prefill));
    dialog('快速收藏案例',`<div class="quick-capture"><div class="quick-capture-step"><b>1</b><label>粘贴来源链接<textarea id="capture-link" class="short" required placeholder="Pinterest、新片场、ShotDeck 或任意公开网页链接">${esc(url)}</textarea></label>${btn('从剪贴板粘贴','radar-capture-paste')}</div><div class="quick-capture-step"><b>2</b><label>说／写下你喜欢什么<textarea id="capture-note" placeholder="例如：喜欢这个光线、人物被空间包围的感觉，以及开场推进镜头。">${esc(note)}</textarea></label>${btn('开始说','radar-capture-voice','aria-pressed="false"')}</div><div class="quick-capture-promise"><strong>其他不用填</strong><span>系统先保存，再补标题、分类、标签和下载提示；整理失败也不会丢掉这条记录。</span></div></div>`,btn('保存到参考库','radar-capture-save','',true));
    setTimeout(()=>$('#capture-link')?.focus(),0);
  }
  function noteJoin(prefill){return prefill.note&&prefill.title?' · ':'';}
  ...
  async function enrich(id){
    ...
    record.sourceSite=info?.siteName||record.sourceSite;record.externalPreview=info?.previewImage||record.externalPreview||'';record.previewDirect=Boolean(record.externalPreview);record.mediaKind=classification.mediaType||info?.mediaKind||'webpage';
  ...
  function save(){
    ...
    if(existing){if(note&&!String(existing.notes||'').includes(note)){existing.captureUserNote=[existing.captureUserNote,note].filter(Boolean).join('\n');existing.notes=[existing.notes,`【再次记录】${note}`].filter(Boolean).join('\n');existing.updatedAt=new Date().toISOString();saveAesthetic(all);}close();route={view:'aesthetic'};render();aestheticDialog(existing);notify('这个链接已经收藏；你的新想法已追加。');return existing;}
    const record=StudioAesthetic.entry(new URL(link).hostname),rule=heuristic(note,link);record.link=link;record.category=rule.category;record.tags=[...rule.tags,'快速收藏'];record.notes=note?`【我喜欢】${note}`:'尚未写喜欢什么，可稍后补充';record.requirements=record.notes;record.captureUserNote=note;record.captureStatus='已保存，正在自动整理';record.sourceSite=new URL(link).hostname;record.sourceUsage='link-only';record.favorite=true;record.folderIds=[];all.push(record);saveAesthetic(all);close();route={view:'aesthetic'};render();notify('已先保存。你可以继续浏览，系统会在后台补标题和分类。');void enrich(record.id);return record;
  }
  ...
  function bookmarklet(){
    const target=PUBLIC_WORKBENCH,code=`javascript:(()=>{const n=window.getSelection?String(window.getSelection()):'';window.open('${target}?capture='+encodeURIComponent(location.href)+'&captureTitle='+encodeURIComponent(document.title)+'&captureNote='+encodeURIComponent(n),'_blank')})()`;
    return code;
  }
  function helperDialog(){
    const code=bookmarklet();dialog('一键收藏助手',`<div class="capture-helper"><section><h3>Mac／电脑浏览器</h3><p>把下面这个按钮拖到浏览器书签栏。以后看到喜欢的网页，点一下就会打开快速收藏窗口，并自动带回当前链接、标题和你选中的文字。</p><a class="button primary" href="${esc(code)}">收藏到 JINHUA</a>${btn('复制收藏按钮代码','radar-capture-copy')}</section><section><h3>iPhone／iPad</h3><p>在原网站点“分享→复制链接”，再打开 JINHUA 的“快速收藏”；点“从剪贴板粘贴”即可。系统不会保存网站密码。</p></section></div>`);
  }
  ...
  function handleIncoming(){
    const params=new URLSearchParams(location.search),url=params.get('capture'),prefill={url,title:params.get('captureTitle')||'',note:params.get('captureNote')||''};
    if(params.get('pinterest')==='connected')setTimeout(()=>pinterestDialog(),250);
    if(url)setTimeout(()=>quickDialog(prefill),250);
    if(url||params.has('pinterest')){params.delete('capture');params.delete('captureTitle');params.delete('captureNote');params.delete('pinterest');history.replaceState(null,'',location.pathname+(params.toString()?'?'+params:'')+location.hash);}
  }
