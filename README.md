...
11. 灵感速记：TMD/My各有独立碎片库，顶部“记灵感”随时进入。语音录制（最长10分钟）、导入录音、文字草稿自动保存、单条AI提炼、勾选2–8条组合、转为新选题；原始记录和提炼历史分开保存。录音存在浏览器IndexedDB，无需模型或上传服务器；完整ZIP备份会包含录音，清理网站数据前务必备份。
...
包括19项 JavaScript、7项 Python 测试：验收规则、PPT 内嵌媒体、文件安全、上传核验、12帧提取、实际25镜合成、旧库迁移、资料提取、碎片提炼和监听保护。`tests/browser.cjs`、`tests/reverse-browser.cjs`、`tests/library-profile-browser.cjs`、`tests/fragments-browser.cjs` 可用 Playwright + 本机 Chrome 运行；使用隔离浏览器、人工合成素材和模拟麦克风，语音识别与AI返回均为明确模拟，不使用真实麦克风或付费AI。`test-output` 样片仅供软件测试，绝非AI创意成果。
