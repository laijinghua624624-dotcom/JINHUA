...
- `mobile.html`：默认打开“选题雷达”，每天展示 3 条与直播预热、活动、舞台和 AI 影像相关的案例方法；也支持文字、语音、图片、视频、文档与链接采集。
- 雷达卡片可保存到 `My·工作` 或 `My·个人`，随后进入同一个随身收件箱；未登录或断网时先保存在当前手机。
- “添加每日提醒到日历”会生成带重复规则的系统日历提醒。这比依赖网页常驻后台更适合 iPhone/iPad；网页不会在后台自行运行或抓取第三方网站。
- 离线草稿：断网也能先保存，可逐条删除。
