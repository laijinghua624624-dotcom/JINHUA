...
const state={tab:location.hash==='#capture'?'capture':'radar',kind:'text',session:null,items:[],projects:[],localItems:[],pending:0,message:'',recording:null,recordedFile:null,shared:{title:'',body:'',url:''},radarOffset:0,radarCurrent:[],radarSeen:[],radarRemaining:0,radarReset:false,radarWorkspace:localStorage.getItem('jinhua_radar_workspace')||'xinxuan',reminderTime:localStorage.getItem('jinhua_radar_reminder_time')||'09:30'};
function purgeLegacyReverseJSON(){for(const scope of ['xinxuan','personal']){localStorage.removeItem('xuan_ti_ku_reverse_'+scope);const key='lance_studio_v2_'+scope,stored=localStorage.getItem(key);if(!stored)continue;try{const data=JSON.parse(stored);if(!Array.isArray(data.reverse))continue;data.reverse=data.reverse.filter(r=>!r?.legacyId&&!r?.legacy);for(const r of data.reverse){delete r.history;delete r.legacyId;delete r.legacy;if(r.analysis)delete r.analysis.legacyStoryboard;}delete data.reverseMigrationError;localStorage.setItem(key,JSON.stringify(data));}catch{}}}
...
function radarHistory(){try{const value=JSON.parse(localStorage.getItem('jinhua_mobile_radar_seen')||'[]');return Array.isArray(value)?value:[];}catch{return [];}}
state.radarSeen=radarHistory();
function radarPicks(){if(state.radarCurrent.length)return state.radarCurrent;const day=Math.floor(Date.now()/86400000),batch=Radar.mobileBatch(Radar.CASES,state.radarSeen,day+state.radarOffset*97,3);state.radarCurrent=batch.items;state.radarSeen=batch.seen;state.radarRemaining=batch.remaining;state.radarReset=batch.reset;localStorage.setItem('jinhua_mobile_radar_seen',JSON.stringify(batch.seen));return state.radarCurrent;}
...
function radarCard(item){const saved=radarSaved().includes(item.id),base=location.hostname.endsWith('github.io')?'https://lance-content-studio.onrender.com':'',preview=item.poster?`${base}/api/radar-preview?url=${encodeURIComponent(item.poster)}`:'',visual=preview?`<img src="${esc(preview)}" alt="${esc(item.title)}案例预览" loading="lazy" onerror="this.closest('.radar-mobile-visual').classList.add('image-failed');this.remove()">`:`<div class="radar-mobile-placeholder"><span>${esc(item.visualFocus||item.category)}</span></div>`;return `<article class="radar-mobile-card ${item.visualFocus?'visual-first':''}"><a class="radar-mobile-visual" href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">${visual}<span class="radar-visual-source">${esc(item.site||'案例来源')} · 查看原片 ↗</span></a><div class="radar-mobile-content"><div class="radar-mobile-top"><span class="pill">${esc(item.visualFocus||item.category)}</span><small>${esc(item.category)} · ${esc(item.type)}</small></div><h2>${esc(item.title)}</h2><p class="radar-mobile-mechanism">${esc(item.mechanism)}</p><dl><div><dt>${item.visualFocus?'画面为什么值得看':'创意为什么成立'}</dt><dd>${esc(item.craft)}</dd></div><div><dt>我可以怎么拍</dt><dd>${esc(item.transfer)}</dd></div></dl><details class="radar-mobile-details"><summary>看片时再看：开场8秒</summary><p>${esc(item.hook)}</p></details><div class="button-row"><a class="secondary mobile-link-button" href="${esc(item.url)}" target="_blank" rel="noopener noreferrer">看原案例 ↗</a><button class="primary" data-action="radar-save" data-id="${esc(item.id)}" ${saved?'disabled':''}>${saved?'已收进灵感':'收下这个选题'}</button></div></div></article>`;}
function radarView(){
  const picks=radarPicks(),today=new Intl.DateTimeFormat('zh-CN',{month:'long',day:'numeric',weekday:'long'}).format(new Date()),visualCount=picks.filter(item=>item.visualFocus).length;localStorage.setItem('jinhua_mobile_radar_last_viewed',new Date().toISOString().slice(0,10));
  return header('VISUAL RADAR','今天先看 3 个视觉方向。','优先给你摄影、灯光、美术、场景与舞台参考；当前一轮看完前不会重复。')+`<section class="radar-mobile-toolbar"><div><span class="radar-date">${esc(today)} · 本轮还有 ${state.radarRemaining} 条未看</span><strong>${visualCount}条视觉 + ${picks.length-visualCount}条创意</strong></div><label>保存到<select id="radar-workspace"><option value="xinxuan" ${state.radarWorkspace==='xinxuan'?'selected':''}>My·工作</option><option value="personal" ${state.radarWorkspace==='personal'?'selected':''}>My·个人</option></select></label></section><section class="mobile-list radar-mobile-list">${picks.map(radarCard).join('')}</section><div class="button-row radar-more"><button class="secondary" data-action="radar-refresh">换一组 · 本轮不重复</button><button class="secondary" data-tab="capture">记录我自己的想法</button></div><section class="mobile-card reminder-card"><div><span class="mobile-kicker">DAILY REMINDER</span><h2>让手机每天提醒你来看</h2><p>添加系统日历提醒后，即使网页没有打开，iPhone／iPad 也会按时提醒；点提醒中的链接直接回到今日雷达。</p></div><label>提醒时间<input id="radar-reminder-time" type="time" value="${esc(state.reminderTime)}"></label><button class="primary" data-action="radar-reminder">添加每日提醒到日历</button><p class="file-note">首次点击会下载日历文件，请在手机上选择“添加全部”。如已添加过，不要重复添加。</p></section>`;
}
...
function downloadRadarReminder(){
  ...
  const ics=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Lance Content Studio//Daily Radar//ZH-CN','CALSCALE:GREGORIAN','BEGIN:VEVENT',`UID:jinhua-daily-radar@${location.hostname||'local'}`,`DTSTAMP:${created}`,`DTSTART;TZID=Asia/Shanghai:${calendarStamp(start)}`,'RRULE:FREQ=DAILY','SUMMARY:Lance 视觉雷达｜今天先看 3 个',`DESCRIPTION:打开 Lance 随身版查看今天的摄影、美术与创意参考。\\n${url}`,`URL:${url}`,'BEGIN:VALARM','TRIGGER:PT0M','ACTION:DISPLAY','DESCRIPTION:今天的 3 条视觉雷达已经准备好','END:VALARM','END:VEVENT','END:VCALENDAR'].join('\r\n');
...
document.addEventListener('click',async event=>{
  ...
  try{
    if(button.dataset.action==='radar-refresh'){state.radarOffset+=1;state.radarCurrent=[];radarPicks();if(state.radarReset)state.message=`上一轮 ${Radar.CASES.length} 条已看完，现在开始新一轮；上一组仍不会立即重复。`;render();return;}
