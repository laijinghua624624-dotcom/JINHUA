...
function radarPicks(){const date=new Date();date.setUTCDate(date.getUTCDate()+state.radarOffset);return Radar.mobileDaily(Radar.CASES,date,3);}
...
function radarView(){
  ...
  return header('VISUAL RADAR','今天先看 3 个视觉方向。','每天至少 2 条聚焦摄影、灯光、美术、场景或舞台视觉；先看如何拍，再看创意机制。')+`<section class="radar-mobile-toolbar"><div><span class="radar-date">${esc(today)}</span><strong>2条视觉 + 1条创意</strong></div><label>保存到<select id="radar-workspace"><option value="xinxuan" ${state.radarWorkspace==='xinxuan'?'selected':''}>My·工作</option><option value="personal" ${state.radarWorkspace==='personal'?'selected':''}>My·个人</option></select></label></section><section class="mobile-list radar-mobile-list">${picks.map(radarCard).join('')}</section><div class="button-row radar-more"><button class="secondary" data-action="radar-refresh">换一组视觉</button><button class="secondary" data-tab="capture">记录我自己的想法</button></div><section class="mobile-card reminder-card"><div><span class="mobile-kicker">DAILY REMINDER</span><h2>让手机每天提醒你来看</h2><p>添加系统日历提醒后，即使网页没有打开，iPhone／iPad 也会按时提醒；点提醒中的链接直接回到今日雷达。</p></div><label>提醒时间<input id="radar-reminder-time" type="time" value="${esc(state.reminderTime)}"></label><button class="primary" data-action="radar-reminder">添加每日提醒到日历</button><p class="file-note">首次点击会下载日历文件，请在手机上选择“添加全部”。如已添加过，不要重复添加。</p></section>`;
}
