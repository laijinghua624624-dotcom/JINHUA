...
(async()=>{
 ...
 const global=()=>page.evaluate(()=>JSON.parse(localStorage.getItem('lance_studio_aesthetic')));
 await page.goto('http://127.0.0.1:8000/');await nav('aesthetic');assert.equal((await global()).length,1);
 await page.reload();await nav('aesthetic');assert.equal((await global()).length,1);
 ...
 await nav('aesthetic');await page.locator('.aesthetic-card').filter({has:page.getByRole('heading',{name:'测试摄影收藏',exact:true})}).getByRole('button',{name:'查看／编辑',exact:true}).click();await page.locator('[name="aes-project"]').check();await click('保存参考');
 ...
 await page.setViewportSize({width:1440,height:1000});await page.locator('#scope').selectOption('xinxuan');await nav('profile');assert.ok(!(await page.locator('#view').innerText()).includes('synthetic-profile.txt'));await nav('supply');await page.locator('[data-field="inspirationBrief.theme"]').fill('测试工作主题');await page.locator('[data-field="inspirationBrief.theme"]').blur();
 ...
 await page.unroute('**/api/chat');await page.route('**/api/chat',r=>r.fulfill({json:{text:'{"ideas":[]}'}}));await nav('supply');await click('生成一批新方向');await idle();assert.equal((await read('xinxuan')).inspirations.length,6);
 await page.reload();await nav('supply');assert.equal(await page.locator('.grid .card').count(),6);assert.deepEqual(errors,[]);
 console.log('PASS: legacy/shared library, links/categories/favorites, real bulk/append uploads, unsaved metadata, My 7 scripts, profile extraction+consent+scope isolation, selected project image context, TMD mocked supply+error preservation, reload, mobile.');
