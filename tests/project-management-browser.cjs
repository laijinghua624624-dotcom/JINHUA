...
(async()=>{const browser=await chromium.launch({headless:true,executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});try{
  ...
  assert.equal(await page.locator('.project-section').getByText('中秋直播专场',{exact:true}).count(),1);
  ...
  let stored=await page.evaluate(()=>JSON.parse(localStorage.getItem('lance_studio_v2_xinxuan')));
  ...
  await page.getByRole('button',{name:'继续项目',exact:true}).click();
  await page.getByRole('button',{name:'项目设置',exact:true}).click();
  await page.getByText('取消或删除项目',{exact:true}).click();
  await page.getByRole('button',{name:'取消项目',exact:true}).click();
  await page.locator('#project-cancel-reason').fill('档期变化');
  await page.getByRole('button',{name:'确认取消项目',exact:true}).click();
  assert.equal(await page.locator('.project-section').getByText('中秋直播专场',{exact:true}).count(),1);
  assert.equal(await page.getByText('取消说明：档期变化',{exact:true}).count(),1);
  await page.getByRole('button',{name:'重新启动',exact:true}).click();
  await page.getByRole('button',{name:'继续项目',exact:true}).click();
  await page.getByRole('button',{name:'项目设置',exact:true}).click();
  await page.getByText('取消或删除项目',{exact:true}).click();
  await page.getByRole('button',{name:'删除项目',exact:true}).click();
  await page.getByRole('button',{name:'移到项目回收站',exact:true}).click();
  stored=await page.evaluate(()=>JSON.parse(localStorage.getItem('lance_studio_v2_xinxuan')));
  assert.equal(stored.projects.some(p=>p.title==='中秋直播专场'),false);
  assert.equal(stored.topics.some(t=>t.title==='月光回家'),false);
  assert.equal(stored.assets.some(a=>a.name==='月光舞台参考'),false);
  assert.equal(stored.projectTrash.length,1);
  assert.equal(stored.projectTrash[0].topics[0].title,'月光回家');
  await page.getByRole('button',{name:'恢复项目',exact:true}).click();
  stored=await page.evaluate(()=>JSON.parse(localStorage.getItem('lance_studio_v2_xinxuan')));
  assert.equal(stored.projectTrash.length,0);
  assert.equal(stored.projects.some(p=>p.title==='中秋直播专场'),true);
  assert.equal(stored.topics.some(t=>t.title==='月光回家'),true);
  assert.equal(stored.assets.some(a=>a.name==='月光舞台参考'),true);
  assert.deepEqual(errors,[]);
  console.log('PASS: archive, cancel, trash and restore preserve complete project data');
