...
(async()=>{const browser=await chromium.launch({headless:true,channel:'chrome'});try{
 ...
 await page.goto('http://127.0.0.1:8000/');await page.locator('nav [data-view="aesthetic"]').click();
 ...
 await page.goto('http://127.0.0.1:8000/legacy.html');assert.equal(await page.locator('#apiKey').isDisabled(),true);assert.match(await page.locator('meta[http-equiv="Content-Security-Policy"]').getAttribute('content'),/connect-src 'none'/);
