...
(async()=>{
  const browser=await chromium.launch({headless:true,executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'}),page=await browser.newPage({viewport:{width:390,height:844}}),errors=[];page.on('pageerror',error=>errors.push(error.message));
  await page.goto('http://127.0.0.1:8787/mobile.html');assert.equal(await page.getByRole('heading',{name:'今天先看 3 个视觉方向。'}).count(),1);assert.equal(await page.locator('.radar-mobile-card').count(),3);assert.ok(await page.locator('.radar-mobile-card.visual-first').count()>=2);assert.ok(await page.locator('.radar-mobile-visual img').count()>=2);assert.equal(await page.locator('.radar-mobile-cn').count(),3);assert.equal(await page.getByRole('button',{name:'收下这个选题'}).count(),3);assert.equal(await page.getByRole('button',{name:'添加每日提醒到日历'}).count(),1);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+2),false);
  ...
  assert.deepEqual(errors,[]);await browser.close();console.log('PASS: mobile daily radar, reminder entry, offline capture, responsive layout, and desktop inbox setup state.');
