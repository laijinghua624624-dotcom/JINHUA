...
(async()=>{
  ...
  await page.goto('http://127.0.0.1:8787/mobile.html');assert.equal(await page.getByRole('heading',{name:'今天先看 3 个。'}).count(),1);assert.equal(await page.locator('.radar-mobile-card').count(),3);assert.equal(await page.getByRole('button',{name:'收下这个选题'}).count(),3);assert.equal(await page.getByRole('button',{name:'添加每日提醒到日历'}).count(),1);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth+2),false);
