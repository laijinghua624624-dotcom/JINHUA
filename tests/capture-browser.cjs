...
(async()=>{
  ...
  await page.goto(base+'?capture='+encodeURIComponent('https://example.com/another')+'&captureNote='+encodeURIComponent('喜欢这个构图'));
  await page.getByRole('heading',{name:'快速收藏案例'}).waitFor();assert.equal(await page.locator('#capture-link').inputValue(),'https://example.com/another');assert.equal(await page.locator('#capture-note').inputValue(),'喜欢这个构图');
