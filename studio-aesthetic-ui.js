...
let aestheticQuery={category:'全部',coverType:'全部',tag:'全部',search:'',favorite:false,page:1};
...
function coverReferenceSubnav(all,q,folders){const types=['全部','过往封面','人物照片','纯字体','增量优化'];return `<div class="cover-reference-subnav"><div><strong>封面参考</strong><small>作为每条视频汇报的风格锚点：保留账号识别，只做与本条内容有关的增量优化。</small></div><div class="actions">${types.map(type=>btn(`${type==='全部'?'全部封面':type} <small>${all.filter(item=>A.coverSubtypeMatch(item,type,folders)).length}</small>`,'aesthetic-cover-type',`data-cover-type="${esc(type)}" class="${(q.coverType||'全部')===type?'active':''}"`)).join('')}</div></div>`;}
function currentAestheticQuery(){return {...aestheticQuery,coverFolders:coverReferenceFolders()};}
function renderAesthetic(){
  const all=folderLibraryEntries(globalAssets()),coverFolders=coverReferenceFolders(),trashCount=loadAestheticTrash().length,q=aestheticQuery,items=A.viewFilter(all,currentAestheticQuery()),categories=[...new Set([...A.CATEGORIES,...all.map(a=>A.categoryOf(a,coverFolders))])],tags=[...new Set(all.flatMap(a=>a.tags))].sort();
...
async function aestheticAction(action,e){
  ...
  if(action==='aesthetic-category'){aestheticQuery.category=e.dataset.category;aestheticQuery.coverType='全部';aestheticQuery.page=1;render();return;}
  if(action==='aesthetic-cover-type'){aestheticQuery.category='封面参考';aestheticQuery.coverType=e.dataset.coverType||'全部';aestheticQuery.page=1;render();return;}
