...
function radarCaseCard(item,featured=false){
  ...
  return `<article class="radar-case ${featured?'featured':''}"><a class="radar-case-visual" href="${esc(A.link(item.url))}" target="_blank" rel="noopener noreferrer">${visual}<span class="radar-case-source">${esc(item.site||'案例来源')} · 查看原片 ↗</span></a><div class="radar-case-body"><div class="radar-card-top"><span class="badge">${esc(item.visualFocus||item.category)}</span><small>${esc(item.category)} · ${esc(item.type)}</small></div><h3>${esc(item.title)}</h3><p class="radar-mechanism">${esc(item.mechanism)}</p><dl><div><dt>为什么值得看</dt><dd>${esc(item.craft||item.goal)}</dd></div><div><dt>开场8秒</dt><dd>${esc(item.hook)}</dd></div><div><dt>怎么转成我的</dt><dd>${esc(item.transfer)}</dd></div></dl><div class="actions">${radarOpenLink(item.url,'看原案例')}${saved?btn('已收藏 · 整理文件夹','radar-open-saved',`data-id="${esc(saved.id)}"`):btn('收藏到审美库','radar-save-case',`data-id="${esc(item.id)}"`,true)}${btn('转为我的选题','radar-topic',`data-id="${esc(item.id)}"`)}</div></div></article>`;
}
function renderRadar(){
  ...
  return hero('CASE RADAR','案例雷达','先看画面，再判断值不值得看片；图片只用于预览，最终以来源页完整作品为准。',btn('记录我发现的案例','radar-new','',true)+btn('打开已收藏','nav','data-view="aesthetic"'))+
